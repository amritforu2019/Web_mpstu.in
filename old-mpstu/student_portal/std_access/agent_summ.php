...
location.href='agent_summ?id='+val;
      ...
      <tr>
        <td colspan="8" align="left"   valign="top" bgcolor="#FFFFFF" >Select Customer Name
          <select name="party" id="party"  onchange="javascript:change_page1(this.value)"  >
            ...
            <?php
    $qry=mysqli_query($DB_LINK,"select * from agent_reg");
	while($r=mysqli_fetch_array($qry))
	{?>
            <option value="<?php echo $r["id"];?>"
	<?php if($_REQUEST['id']==$r['id']) echo 'selected'; ?> ><?php echo $r["name"]?> [<?php echo $r["contact"]?> ]</option>
            <?php } ?>
          </select>
...
$q=mysqli_query($DB_LINK,"SELECT * FROM `leg_add` where member='".$_REQUEST['id']."' order by id asc ");
...
$di=mysqli_query($DB_LINK,"SELECT * FROM agent_reg where id='".$row['member']."' ");	
...
mysqli_query($DB_LINK,"update `leg_add` set `name`='".$rowm['name']."' where id='".$row['id']."' ");
