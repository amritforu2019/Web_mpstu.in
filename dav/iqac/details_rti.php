...
<? if($_REQUEST['pid']) {
   ...
   <figure class="col-md-4 col-sm-4 col-xs-12 gallbx">
      <a class="fancybox" href="upload/gallery/thumb/<? echo $row['images']; ?>" data-fancybox-group="gallery" title="">
  ...
  <? $i++; $k++;}}?>
	  ...
	  <? } else { ?>
           <?
					...
					$qry1=mysql_query("select *  from photogallery_cat where status=1 and status2=1");
					$qry=mysql_query("select * from photogallery_cat where status=1 and  status2=1 order by id "); 
					$reccnt=mysql_num_rows($qry1);   
				?> 	 
						<? if($reccnt!=0){?>
		 ...
		 <figure class="col-md-4 col-sm-4 col-xs-12 gallbx">
      <a class="fancybox" href="details_rti?pid=<?=$id?>" data-fancybox-group="gallery" title="">
